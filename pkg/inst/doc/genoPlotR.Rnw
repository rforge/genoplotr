...
start1, end1, start2, end2 and col. In the first comparison, the start
and stops correspond to the genes created above.
...
Many options can be added to this simple plot. First, the first comparison
will be colored in grey scale, using an arbitrary scale passed to the function
\code{apply\_color\_scheme}.
...
Finally, the plot is drawn, addind a title.
...
perfectly that part or all of this example becomes obsolete. Please contact
the author of this document in such a case.
...
There, the genomes can be aligned by entering the two genomes accession 
...
under the correct names, they can be parsed directly by \genoPlotR{}.

<<ex_2_load_fake, results=hide, eval=FALSE>>=
BH <- try(read_dna_seg_from_file("NC_005966.gbk"))
BQ <- try(read_dna_seg_from_file("NC_005955.gbk"))
BH_vs_BQ <- try(read_comparison_from_blast("NC_005966_vs_NC_005955.blast"))
@ 
